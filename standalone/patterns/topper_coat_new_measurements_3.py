...
class Design(designBase):
    ...
    def pattern(self):
        ...
        FUS = A.addPoint('FUS', left(FUC, CD.highbust_arc_f/2.0)) #temp front upperbust side
        FAP = A.addPoint('FAP', left(FUC, CD.armfold_to_armfold_f/2.0)) #front armfold point 
                
        FBC = A.addPoint('FBC', down(FNC, CD.neck_front_to_bust_f)) #front bust center
        FBP = A.addPoint('FBP', left(FBC, CD.bustpoint_to_bustpoint/2.0)) #front bust point
        FBS1 = A.addPoint('FBS1', left(FBC, CD.bust_arc_f/2.0)) #temp front bust side 
        length = CD.neck_back_to_bust_b - CD.neck_back_to_highbust_b
        FBS = A.addPoint('FBS', lowestP(intersectCircles(FBP, (CD.bust_arc_f/2.0 - CD.bustpoint_to_bustpoint/2.0), FUS, (CD.neck_back_to_bust_b - CD.neck_back_to_highbust_b)))) #front bust side 
       
        FWS1 = A.addPoint('FWS1', left(FWC, CD.waist_arc_f/2.0)) #temporary front waist side 1 - on waist line
        FWS2 = A.addPoint('FWS2', onLineAtLength(FUS, FBS, CD.armpit_to_waist_side)) #temporary front waist side 2 - on side seam
        FHC = A.addPoint('FHC', down(FWC, CD.waist_to_hip_f)) #front hip center        
        FHS = A.addPoint('FHS', left(FHC, CD.hip_arc_f/2.0)) #front hip side
        ...
        pnt = onLineAtY(a3, FBP, FWC.y)
        a11 = A.addPoint('a11', extendLine(a3, pnt, 0.10 * distance(FWC, a_H1))) #pocket center
        a12 = A.addPoint('a12', onLineAtLength(a_H2, g4, 0.5 * distance(FWC, a_H1))) #pocket side         
