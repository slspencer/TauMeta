...
class Design(designBase):
    ...
    def pattern(self) :
        ...
        shoulder_curve_angle = angleOfDegree(5)
        ...
        p12b.addOutpoint(polar(p12b, distance(p12b, p13b) / 3.0, angleOfLine(p12b, p13b) + shoulder_curve_angle))
        p13b.addInpoint(polar(p13b, distance(p12b, p13b) / 3.0, angleOfLine(p13b, p12b) - shoulder_curve_angle))        
        ...
        p37 = B.addPoint('p37', up(p32, CD.front_waist_length))
        ...
        p41 = B.addPoint('p41', intersectLines(p40,p36, p38, p37))
        p44 = B.addPoint('p44', extendLine(p38, p41, distance(p38, p41)))
        p41b = B.addPoint('p41b', midPoint(p41, p44))        
        ...
        BD1 = B.addPoint('BD1', intersectLineRay(p41, p45, p44, angleOfLine(p41, p44) + ANGLE90))        
        BD1.i = B.addPoint('BD1.i', onLineAtLength(p44, p41, 0.75*CM)) 
        BD1.o = B.addPoint('BD1.o', rotate(BD1, BD1.i, angleOfDegree(8)))
        #lapel point               
        #p42 = B.addPoint('p42', extendLine(p38, p37, 1.*CM))
        #p43 = B.addPoint('p43', right(p31, 3.*CM))
        #p44 = B.addPoint('p44', midPoint(p41, p42))        
        p42 = B.addPoint('p42', extendLine(p44, p37, 0.8 * distance(p38, p44))) 
        #p42b = B.addPoint('p42b', down(p42, distance(p37, p31)/8.))
        p42b = B.addPoint('p42b', onRayAtX(BD1.o, angleOfLine(BD1, BD1.o) + ANGLE90, p42.x))                            
        p43 = B.addPoint('p43', right(p31, distance(p38, p44)))
        ...
        #p42.addOutpoint(onLineAtLength(p43, p43.inpoint, distance(p42, p43)/3.0))        
        p42.addOutpoint(polar(p42, distance(p42, p43)/3.0, angleOfDegree(45)))
        p42b.addOutpoint(down(p42b, distance(p42b, p33)/3.0))                
        p43.addInpoint(up(p43, distance(p42, p43)/6.0))
        p43.addOutpoint(down(p43, distance(p43, p33)/3.0))                                  
        p36.addInpoint(polar(p36, distance(p43, p36)/3.0, angleOfLine(p36, p43.outpoint)))
        p45.addInpoint((p43.x, p45.y))
        p45.addOutpoint(extendLine(p36, p45, distance(p45, p33)))
        p33.addInpoint(polar(p33, distance(p43, p33)/6.0, angleOfLine(p33, p41) + ANGLE90))
        p33.addOutpoint(extendLine(p36, p33, distance(p33, p34) / 3.0))
        p48.addInpoint(p35)
        ...
        p29.addOutpoint(polar(p29, distance(p29, p27)/3.0, angleOfLine(p29, p27) - shoulder_curve_angle))
        p27.addInpoint(polar(p27, distance(p29, p27)/3.0, angleOfLine(p27, p29) + shoulder_curve_angle))
        ...
        p53 = C.addPoint('p53', (p42.x, p28.y))

        curve = points2List(p27, p27.inpoint, p29.outpoint, p29)
        pCCB = C.addPoint('pCCB', onCurveAtLength(curve, distance(p27, p28)/4.0))
        ...
        back_neck_curve = points2List(pA, pA.outpoint, p12b.inpoint, p12b) 
        pnt = extendLine(p51, pCCB, curveLength(back_neck_curve))    

        pCC = C.addPoint('pCC', polar(pnt, 5.*CM, angleOfLine(p51, pnt) - ANGLE90))                
        pCD = C.addPoint('pCD', polar(pnt, 7.*CM, angleOfLine(p51, pnt) + ANGLE90))
        pCE = C.addPoint('pCE', polar(pCD, 1.*CM, angleOfLine(pnt, pCD) - ANGLE90))
        pCF = C.addPoint('pCF', intersectLineRay(p27, p29, pCC, angleOfLine(pnt, p51)))
        pCB = C.addPoint('pCB', intersectLines(p51, pnt, pCC, pCE))                
                
        #control points 
        #undercollar rolline curve             
        pCB.addOutpoint(polar(pCB, distance(p51, pCB)/3.0, angleOfLine(pCC, pCE) + ANGLE90))
        print 'F'         
        p51.addInpoint(dPnt((p51.x, p53.y)))      
        print 'G'
        #undercollar        
        ...
        p53.addInpoint(polar(p53, distance(p53, pCE)/6.0, angleOfLine(p51, pCE)))
        p53.addOutpoint(polar(p53, distance(p53, p41b)/3.0, angleOfLine(p53.inpoint, p53))) 
        p41b.addInpoint(polar(p41b, distance(p53, p41b)/6.0, angleOfLine(p41b, p41) + ANGLE90))        
        print 'C'               
        ...
        BG1 = dPnt((p41.x, p31.y))
        BG2 = down(BG1, distance(p31, p33))
        B.addGrainLine(BG1, BG2)
        
        path = (['M', FSH, 'L', p6, 'L', p34, 'L', p37, 'L', p38, 'L', p27, 'L', FSH])   
        B.addGridLine(path)
        
        path = (['M', pN, 'L', pM, 'L', pO, 'C', pV, 'L', pW, 'L', pN, 'M', p41, 'L', p33])             
        B.addFoldLine(path)
        ...
        path = (['M', p27, 'C', p41, 'L', BD1.i, 'L', BD1.m, 'L', BD1.o, 'L', p42b,
                       'C', p33, 'C', p48, 'L', p24,
                       'L', p23, 'C', p22, 'C', p20, 'C', p21, 
                       'C', pJ, 'C', pI, 'C', pH, 'C', p29, 'C', p27])                       
        B.addSeamLine(path)
        ...
        CG2 = dPnt(polar(CG1, distance(pCB, p51) / 2.0, angleOfLine(pCB, pCB.outpoint)))
        C.addGrainLine(CG1, CG2)
        
        C.addFoldLine(['M', pCB, 'C', p51])

        #path = (['M', pCE, 'C', p53, 'L', p51, 'C', pCF, 'L', pCC, 'L', pCB, 'L', pCE])        
        path = (['M', pCE, 'C', p53, 'C', p51, 'C', pCF, 'L', pCC, 'L', pCE]) 
        C.addSeamLine(path)
