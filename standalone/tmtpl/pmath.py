...
def outsetCurve(curve, outset_width, outset_angle):
    #single curve - update Topper Coat to use outsetCurves()
...
def outsetCurves(orig_curve, outset_width, outset_angle):
    #accepts array curve[] which contains cubic bezier [p1, c1, c2, p2], and the angle of outset (ANGLE90 or -ANGLE90)
    #returns outset_curve[] which contains two cubic bezier curves [[p1, oc1, oc2, op2, oc3, oc4, p2])
        
    #split the original curve
    new_curves = splitCurveAtLength(orig_curve, curveLength(orig_curve)/2)
    curve1 = points2List(new_curves[0], new_curves[1], new_curves[2], new_curves[3])
    curve2 = points2List(new_curves[3], new_curves[4], new_curves[5], new_curves[6])
    
    ocurve1 = []
    ocurve2 = []
    
    #get outset points for each new curve
    for (curve, outset_curve) in (curve1, ocurve1), (curve2, ocurve2):

        P1 = curve[0]
        C1 = curve[1]
        C2 = curve[2]
        P2 = curve[3]
        #get angles to create outset points
        angle1 = angleOfLine(P1, C1) + outset_angle 
        angle2 = angleOfLine(C2, P2) + outset_angle
        angle3 = angleOfLine(C1, C2) + outset_angle 
        #get initial outset curve points
        OP1 = dPnt(polar(P1, outset_width, angle1))
        oc1a = polar(C1, outset_width, angle1)
        oc1b = polar(C1, outset_width, angle3)
        oc2b = polar(C2, outset_width, angle3)
        oc2a = polar(C2, outset_width, angle2)
        OP2 = dPnt(polar(P2, outset_width, angle2))
        #calculate intersection of control handles
        OC1 = dPnt(intersectLines(OP1, oc1a, oc1b, oc2b))
        OC2 = dPnt(intersectLines(oc1b, oc2b, oc2a, OP2))
        
        for item in [OP1, OC1, OC2, OP2]:
            outset_curve.append(item)
            print item.x, item.y
    
    ocurve = ocurve1 + ocurve2[1:] #exclude the duplicate 1st element in ocurve2
...
def intersectLines(p1, p2, p3, p4):
    ...
    elif (p1.x == p2.x): #if 1st line vertical, use slope of 2nd line
        x = p1.x
...
def splitCurveAtLength(curve, length):
    '''Accepts a curve list with points P0 C1 C2 P1, and a length.
...
def tangentOnCircleFromPoint(C, r, P):
    ...
    Distance from P to C must be greater than r
    '''
    ##print('C:', C.x, C.y)
    ##print('P:', P.x, P.y)
    dPnt(C)
    ...
    #if radius less than distance between points then no tangent exists
    if r > d:
        print('No Tangent Found: tangentOnCircleFromPoint( C =', C.x, C.y, 'r =', r, 'P =', P.x, P.y)
    try:
        l = sqrt(d*d - r*r)
    except:
        print 'No Tangent Found: tangentOnCircleFromPoint: circles ', 'd*d - r*r = ', d*d, '-', r*r, '=', d*d - r*r
        raise
