...
def pPointXY(parent,id,x,y,transform=''):
    ...
    p1=PointXY('reference',id,x,y,'point_style',transform)
    parent.add(p1)
    return p1
...
def pPoint(parent,id,p1,transform=''):
    ...
    return pPointXY(parent,id,p1.x,p1.y,transform='')
...
def cPointXY(parent,id,x,y,transform=''):
    ...
    pnt=PointXY('reference',id,x,y,'controlpoint_style',transform)
    parent.add(pnt)
    return pnt

def cPoint(parent,id,pnt,transform=''):
    '''Accepts parent object,id,point object (can be from another calculation),and optional transform.
    Returns object of class Point. Creates SVG blue open dot in reference group for control point in bezier curves.'''
    return cPointXY(parent,id,pnt.x,pnt.y,transform='')

#---calculate points---

def updatePoint(p1,p2):
    '''
    Accepts pnt and new_pnt of class Pnt.
    pnt is updated with x & y values from new_pntT
    '''
...
def symmetricPoint(p1,p2,type='vertical'):
    """
    Accepts p1 and p2 of class Pnt, and optional arg type is either 'vertical' or 'horizontal'
    Returns p3 of class Pnt which is "mirror image" of p1 relative to p2
...
def polarPoint(p1,distance,angle):
...
def xyMidPointXY(x1,y1,x2,y2,n=0.5):
    '''Accepts x1,y1,x2,y2 and 0<n<1,returns x,y'''
    return (x1+x2)*n,(y1+y2)*n

def xyMidPoint(p1,p2,n=0.5):
    '''Accepts p1 & p2 and 0<n<1,returns x,y'''
    return xyMidPointXY(p1.x,p1.y,p2.x,p2.y,n)

def pntMidPointXY(x1,y1,x2,y2,n=0.5):
    ...
    pnt=Pnt()
    pnt.x,pnt.y=xyMidPointXY(x1,y1,x2,y2,n)
    return pnt

def pntMidPoint( p1,p2,n=0.5):
    '''Accepts p1 & p2 and 0<n<1,returns p3'''
    pnt=Pnt()
    pnt.x,pnt.y=xyMidPoint(p1,p2,n)
    return pnt

# ----------------...Calculate intercepts given x or y..------------------------------

def intersectLineAtXP(p1,p2,x):
        ...
        if ((x >=xmin) and (x <=xmax)):
            while (i<(len(interpolatedPoints)-1)):
                if (x >=interpolatedPoints[i].x) and (x <=interpolatedPoints[i+1].x):
                    pnt=intersectLineAtXP(interpolatedPoints[i],interpolatedPoints[i+1],x)
...
def distanceXY(xstart,ystart,xend,yend):
    ...
    while j <=len(curve) -4 and found !='true': # for each bezier curve in curveArray until a point is found
        ...
        if intersection_estimate !=None or intersection_estimate !='':
            ...
            while (k<len(interpolated_points)-1) and (found !='true'):
                pnt_on_line=polarPoint(fixed_pnt,distance(fixed_pnt,interpolated_points[k]),angle)
...
def curveLengthAtPointXY(pnt,curve,n=100):
    ...
    while j <=len(curve) -4 and found==0: # for each curve,get segmentLength & add to curveLength
        ...
        while (i <=n) and (found==0):
                segmentLength=segmentLength+distance(interpolatedPoints[i-1],interpolatedPoints[i]) #length from previous point to current point
                try:
                    if pnt==interpolatedPoints[i] or distance(pnt,interpolatedPoints[i]) <=0.01:
                        found=1
                except:
...
def interpolatedCurveLengthAtPointXY(pnt,interpolatedPoints):
    # add up lengths between the interpolated points
    segmentLength=0.0
    found=0
    i=1
    while (i<len(interpolatedPoints)) and (found==0):
            segmentLength=segmentLength+distance(interpolatedPoints[i-1],interpolatedPoints[i]) #length from previous point to current point
            try:
                if pnt==interpolatedPoints[i] or distance(pnt,interpolatedPoints[i]) <=1.0:# 1 pixel
                    found=1
            except:
                print 'out of bounds i=',i
            i=i+1
...
def intersectCircleAtX(C,r,x):
    ...
    P=[]
    if abs(x-C.x)>r:
        print 'x is outside radius of circle in intersections.intersectCircleAtX()'
    else:
...
def splitCurveAtLength(length,curve):
    ...
    curve3.c1=polarPoint(split_pnt,length,forward_tangent_angle)
    curve3.c2=polarPoint(curve3,length,angleOfLine(curve3,curve[2])) # preserve angle b/w original 2nd control point & P1
...
def waistDart(parent,dart_width,dart_length,length,waist_curve,dart_angle=ANGLE90):
    ...
    #dart_apex=pPoint(parent,parent.name+'dart_apex',polarPoint(split_curve[3],dart_length,angleOfLine(split_curve[3],split_curve[2])+ANGLE90))
    ...
    dart_apex=polarPoint(split_curve[3],dart_length,angleOfLine(split_curve[3],split_curve[2])+dart_angle)
...
def addDartFold(parent,dart,inside_pnt):
        ...
        temp_pnt=pntMidPoint(dart.i,dart.o)
...
def addDartMidPointXY(parent,dart_leg1,dart_apex,dart_leg2,next_pnt):
        '''old version of addDartFold() -- uses .o,oc,.i,.ic instead of newer .o,.oc,.i,.ic'''
        DART_LENGTH=distance(dart_apex,dart_leg1)
        DART_HALF_ANGLE=abs(angleOfVector(dart_leg1,dart_apex,dart_leg2))/2.0
        DART_LEG1_ANGLE=angleOfLine(dart_apex,dart_leg1)
        DART_LEG2_ANGLE=angleOfLine(dart_apex,dart_leg2)
        # determine which direction the dart will be folded
        if DART_LEG2_ANGLE <=DART_LEG1_ANGLE:
            DART_FOLD_ANGLE=DART_LEG2_ANGLE-DART_HALF_ANGLE
        else:
            DART_FOLD_ANGLE=DART_LEG2_ANGLE+DART_HALF_ANGLE
        midpnt=pntMidPoint(dart_leg1,dart_leg2)
        foldpnt=polarPoint(dart_apex,DART_LENGTH,DART_FOLD_ANGLE)
        intpnt=pntIntersectLinesP(dart_leg2,next_pnt,dart_apex,foldpnt)
        # dart midpoint at waist
        pnt=intersectLineAtLength(dart_apex,midpnt,distance(dart_apex,intpnt))
        if hasattr(dart_apex,'m'):
            dart_apex.m.x,dart_apex.m.y=pnt.x,pnt.y
            dart_apex.m.coords  =str(pnt.x)+","+str(pnt.y)
        else:
            dart_apex.m=pPoint(parent,dart_apex.name+'.m',pnt)
        # dart outside leg at cuttingline
        pnt=intersectLineAtLength(dart_leg1,dart_apex,-SEAM_ALLOWANCE)
        if hasattr(dart_apex,'oc'):
            dart_apex.oc.x,dart_apex.oc.y=pnt.x,pnt.y
            dart_apex.oc.coords  =str(pnt.x)+","+str(pnt.y)
        else:
            dart_apex.oc=pPoint(parent,dart_apex.name+'.oc',pnt)
        # dart inside leg at cuttingline
        pnt=intersectLineAtLength(dart_leg2,dart_apex,-SEAM_ALLOWANCE)
        if hasattr(dart_apex,'ic'):
            dart_apex.ic.x,dart_apex.ic.y=pnt.x,pnt.y
            dart_apex.ic.coords  =str(pnt.x)+","+str(pnt.y)
        else:
            dart_apex.ic=pPoint(parent,dart_apex.name+'.ic',pnt)
...
def controlPoints(name,knots):
    ...
    while (i <=c_num):
        ...
        if (current==c_num):
            ...
            pnt=polarPoint(knots[last_knot],length,angle)
