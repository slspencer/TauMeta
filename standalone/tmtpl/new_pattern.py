...
def symmetricPointP(p1,p2,type='vertical'):
    """
...
def polarPointP(p1,distance,angle):
...
def pntOnLine(p1,p2,distance,rotation=0):
    """
    Accepts p1 and p2 of class Pnt, distance, and an optional rotation angle
    Returns pnt of class Pnt on the line measured from p1 towards p2
    The point is optionally rotated about the first point by the rotation angle in degrees
    """
    pnt=Pnt()
    lineangle=angleOfLine(p1,p2)
    angle=lineangle+(rotation*(math.pi/180))
    x=(distance*math.cos(angle))+p1.x
    y=(distance*math.sin(angle))+p1.y
    pnt=Pnt(x,y)
    return pnt
...
def splitCurveAtLength(length,curve):
    ...
    split_pnt.c1=polarPointP(curve[0],length,angleOfLine(curve[0],curve[1])) # preserve angle b/w P0 & original 1st control point
    ...
    curve3.c2=polarPointP(curve3,length,angleOfLine(curve3,curve[2])) # preserve angle b/w original 2nd control point & P1
...
def waistDart(parent,dart_width,dart_length,length,waist_curve,dart_angle=ANGLE90):
    ...
    #dart_apex=rPointP(parent,parent.name+'dart_apex',polarPointP(split_curve[3],dart_length,angleOfLine(split_curve[3],split_curve[2])+ANGLE90))
    ...
    dart_apex=polarPointP(split_curve[3],dart_length,angleOfLine(split_curve[3],split_curve[2])+dart_angle)
...
def neckDart(parent,dart_width,dart_length,length,neck_curve):
    ...
    dart_apex=rPointP(parent,'dart_apex',polarPointP(split_curve[3],dart_length,angleOfLine(split_curve[3],split_curve[2])+ANGLE90))
...
def addDartFold(parent,dart,inside_pnt):
        ...
        mid_pnt=pntOnLine(dart,temp_pnt,distanceP(dart,fold_pnt))
        if hasattr(dart,'m'):
        ...
        #temp_pnt=pntOnLine(dart.o,dart,-SEAM_ALLOWANCE)
        temp_pnt=polarPointP(dart, distanceP(dart, dart.o)+SEAM_ALLOWANCE, angleOfLine(dart, dart.o))
        if hasattr(dart,'oc'):
        ...
        temp_pnt=pntOnLine(dart.i,dart,-SEAM_ALLOWANCE)
        if hasattr(dart,'ic'):
...
def addDartMidPoint(parent,dart_leg1,dart_apex,dart_leg2,next_pnt):
        ...
        pnt=pntOnLine(dart_apex,midpnt,distanceP(dart_apex,intpnt))
        ...
        # dart outside leg at cuttingline
        pnt=pntOnLine(dart_leg1,dart_apex,-SEAM_ALLOWANCE)
        ...
        # dart inside leg at cuttingline
        pnt=pntOnLine(dart_leg2,dart_apex,-SEAM_ALLOWANCE)
...
def slashAndSpread(pivot,angle,*args):
        ...
        while (i<len(list)):
            ...
            rotated_pnt=polarPointP(pivot,distance,angleOfLine(pivot,pnt)+angle) # angle>0=spread clockwise. angle<0=spread counterclockwise.
